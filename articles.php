...
if(isset($_POST['articlecat_id'])) {
    $articlecat_id = $_POST['articlecat_id'];
} elseif(isset($_GET['articlecat_id'])) {
    $articlecat_id = $_GET['articlecat_id'];
} else {
    $articlecat_id = "";
}
if(isset($_POST['p'])) {
...
if(is_numeric($_POST['article_competition']) && $_POST['article_competition'] != 'winner') {
    $cmp = $_POST['article_competition'];
    $criterias[] = "article_competition = ".$cmp;
    $smarty->assign('articlecompetition', $cmp);
}

if($_POST['article_competition'] == 'winner') {
    $cmp_winner = $_POST['article_competition'];
    $smarty->assign('articlecompetition', $cmp_winner);
}
...
if($di_mcategory == 2) {
    $where = ' se_users.user_level_id != 3 AND ';
}
else {
    $where = ' se_users.user_level_id = 3 AND ';
}
...
// GET TOTAL ARTICLES
$total_articles = $article->article_total($where, 1);
...
// GET ARTICLES
$article_array = $article->article_list($page_vars[0], $articles_per_page, $sort, $where, 1, $cmp_winner);
...
$result = $database->database_query("SELECT article_title, article_body, article_id FROM se_articles WHERE article_approved = 1 ORDER BY article_views DESC LIMIT 5");
while($rez = $database->database_fetch_assoc($result)) {
    $rez['article_body'] = str_replace("\r\n", "",html_entity_decode($rez['article_body']));
    $popular_articles[] = $rez;
}
...
$smarty->assign('pplrarticles', $popular_articles);
