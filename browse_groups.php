...
/* $Id: browse_groups.php 80 2009-03-06 02:46:35Z john $ */
...
if(isset($_POST['user_id'])) { $user_id = $_POST['user_id']; } elseif(isset($_GET['v'])) { $user_id = $_GET['user_id']; }
...
//  $page = "error";
//  $smarty->assign('error_header', 639);
//  $smarty->assign('error_message', 12345);
//  $smarty->assign('error_submit', 641);
//  include "footer.php";
//}
...
//if($user_id){
//        $where .= ' AND (SELECT true FROM se_groupmembers WHERE se_groupmembers.groupmember_user_id = '.$user_id.') ';
//
//}
...
//print_r($user_id);
// GET TOTAL GROUPS
$total_groups = $group->group_total($wher);
...
// GET GROUP ARRAY
$group_array = $group->group_list($page_vars[0], $groups_per_page, $s, $where, TRUE, $user_id);
