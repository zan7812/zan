...
if($setting[setting_forum_status] == 2 && (!$user->user_exists || ($user->user_exists && !$forum->forum_is_moderator($user->user_info[user_id])))) {
...
if($topic_id != 0) {
    ...
    if($post_id != 0) {
        ...
        if($database->database_num_rows($post_query) == 1) {
            ...
            if($first_in_topic[forumpost_id] == $post_info[forumpost_id]) {
                $show_title = true;
            } else {
                $show_title = false;
            }
...
if(!$user->user_exists) $forum_permission[allowed_to_post] = 0;
...
if($is_reply && $topic_info[forumtopic_closed]) {
    header("Location: forum_topic.php?forum_id=".$forum_info[forum_id]."&topic_id=".$topic_info[forumtopic_id]);
    exit();
}
if($is_edit && !$forum_permission[allowed_to_editall] && $topic_info[forumtopic_closed]) {
    header("Location: forum_topic.php?forum_id=".$forum_info[forum_id]."&topic_id=".$topic_info[forumtopic_id]);
    exit();
}

// IF EDITING, ENSURE USER IS ALLOWED TO
if($is_edit && (!$user->user_exists || (!$forum_permission[allowed_to_editall] && $user->user_info[user_id] != $post_info[forumpost_authoruser_id]))) {
...
// CREATE NEW TOPIC
if($task == "create") {
    $ratings = $database->database_fetch_assoc($database->database_query("SELECT create_forum FROM se_ratings LIMIT 1"));
    $database->database_query("UPDATE se_users SET user_rating = user_rating + ".$ratings['create_forum']." WHERE user_id = ".$user->user_info['user_id']);
    ...
    if($is_error == 0) {
        header("Location: forum_topic.php?forum_id={$forum_info[forum_id]}&topic_id={$topic_id}");
        exit();
    }
...
// ADD A NEW REPLY
    ...
    // IF NO ERROR, FORWARD TO TOPIC
    if($is_error == 0) {
        header("Location: forum_topic.php?forum_id={$forum_info[forum_id]}&topic_id={$topic_info[forumtopic_id]}&post_id={$post_id}#post_{$post_id}");
        exit();
    }
...
// EDIT A POST
    ...
    // SET ERRORS
    if(trim($topic_title) == "" && $show_title) {
        $is_error = 6000066;
    }
