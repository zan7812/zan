...
/* $Id: group.php 69 2009-02-26 01:22:42Z szerrade $ */
...
// VALIDATE VIEW VAR
if($v != "group" && $v != "members" && $v != "comments" && $v != "photos" && $v != "discussions") {
    $v = "group";
}
...
//        print_r($groupalbum_id.$group->user_rank);
if($task == "album_delete" && $group->user_rank >= 0){
    $groupalbum_id = $_GET['groupalbum_id'];
    $delalbum = $database->database_fetch_assoc($database->database_query("SELECT * FROM se_groupalbums WHERE groupalbum_id = ".$groupalbum_id." LIMIT 1"));
    if($delalbum['groupalbum_user_id'] == $user->user_info['user_id']){
        $database->database_query("DELETE FROM se_groupalbums WHERE groupalbum_id = ".$groupalbum_id." LIMIT 1");
        $database->database_query("DELETE FROM se_groupmedia WHERE groupmedia_groupalbum_id = ".$groupalbum_id." LIMIT 1");
        $database->database_query("DELETE FROM se_groupmediacomments WHERE groupmedia_groupalbum_id = ".$groupalbum_id." LIMIT 1");
    }
}
...
$result = $database->database_query("SELECT groupalbum_id, groupalbum_user_id, groupalbum_datecreated, groupalbum_dateupdated, groupalbum_title, groupalbum_desc,
    groupalbum_views, groupalbum_totalfiles, groupalbum_totalspace, groupalbum_cover FROM se_groupalbums WHERE groupalbum_group_id = ".$group->group_info['group_id']);
while($rez = $database->database_fetch_assoc($result)) {
    if($rez['groupalbum_cover'] == 0){
        $cover = $database->database_fetch_assoc($database->database_query("SELECT groupmedia_id, groupmedia_ext FROM se_groupmedia WHERE groupmedia_groupalbum_id = ".$rez['groupalbum_id']." ORDER BY groupmedia_date DESC LIMIT 1"));
    }
    else{
        $cover = $database->database_fetch_assoc($database->database_query("SELECT groupmedia_id, groupmedia_ext FROM se_groupmedia WHERE groupmedia_groupalbum_id = ".$rez['groupalbum_id']." AND groupmedia_id = {$rez['groupalbum_cover']} LIMIT 1"));
    }
    if($cover['groupmedia_id']) $rez['cover'] = $group->group_dir($group->group_info['group_id']).$cover['groupmedia_id'].'_thumb.'.$cover['groupmedia_ext'];
    else $rez['cover'] = './images/nophoto.gif';
    $rez['groupalbum_totalspace'] = round($rez['groupalbum_totalspace']/1024/1024, 2);
    $group_albums[] = $rez;
}
...
foreach($group_albums as $item) {
    if($item['groupalbum_user_id'] == $user->user_info['user_id']) $create_album = 0;
}
$smarty->assign_by_ref('create_album', $create_album);

// ASSIGN VARIABLES AND DISPLAY GROUP PAGE
$smarty->assign_by_ref('group_albums', $group_albums);
