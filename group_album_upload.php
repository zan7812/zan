...
/* $Id: user_album_upload.php 16 2009-01-13 04:01:31Z john $ */

$page = "group_album_upload";
include "header.php";
...
if($task == 'delete-image') {
    $database->database_query("DELETE FROM se_groupmedia WHERE groupmedia_id = ".$_POST['file_id']." LIMIT 1");
    exit();
}
...
$album = $database->database_query("SELECT * FROM se_groupalbums WHERE groupalbum_id='$album_id' AND groupalbum_user_id='".$user->user_info[user_id]."'");
...
$space_used = $album->group_album_space();
...
if($task == "doupload") {
  ...
  for($f=1;$f<6;$f++)
  {
    ...
    if($_FILES[$fileid]['name'] != "")
    {
      ...
      if( !$file_result[$fileid]['is_error'] )
      {
        ...
        $media_path = $url->url_base.substr($url->url_userdir($user->user_info[user_id]), 2).$file_result[$fileid]['media_id']."_thumb.jpg";
        $media_id = $file_result[$fileid]['media_id'];
        $file_result[$fileid]['file_path'] = '<img src="'.$media_path.'" width="70"/>';
        $local_media_path = substr($url->url_userdir($user->user_info[user_id]), 2).$file_result[$fileid]['media_id']."_thumb.jpg";
        
        if( file_exists($local_media_path) )
        { 
          $media_width = $misc->photo_size($local_media_path, "100", "100", "w");
          $media_height = $misc->photo_size($local_media_path, "100", "100", "h");
          $action_media[] = Array(
            'media_link' => $url->url_create('album_file', $user->user_info[user_username], $album_id, $file_result[$fileid]['media_id']),
  ...
  if($isAjax) {
    ...
    $json = '{"result":"'.$result.'","error":"'.$error.'","size":"'.$size.'", "image_path": "'.$media_path.'","image_id":"'.$media_id.'"}';
