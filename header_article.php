...
$result = $database->database_query("SELECT competition_id, competition_title FROM se_competitions WHERE competition_status = 0 AND competition_category = 2");
while($rez = $database->database_fetch_assoc($result)){
    $article_competitions[] = $rez;
//print_r($rez);
}
if(count($article_competitions)> 0) $smarty->assign('articlecompetitions', $article_competitions);
